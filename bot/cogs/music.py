...
class Player(wavelink.Player):
    ...
    async def add_tracks(self, ctx, tracks):
        ...
        if isinstance(tracks, wavelink.TrackPlaylist):
            ...
            emVar = discord.Embed(title="Song Added",
                                  description=f"Added {tracks[0].title} to the queue.",
                                  color=discord.Color.dark_blue())
            await ctx.send(embed=emVar)
        else:
            if (track := await self.choose_track(ctx, tracks)) is not None:
                self.queue.add(track)
                emVar2 = discord.Embed(title="Track Added",
                                       description=f"Added {track.title} to the queue.",
                                       color=discord.Color.dark_blue())
                await ctx.send(embed=emVar2)
...
class Music(commands.Cog, wavelink.WavelinkMixin):
    ...
    async def connect_command(self, ctx, *, channel: t.Optional[discord.VoiceChannel]):
        ...
        emVar = discord.Embed(title="Joined",
    ...
    async def connect_command_error(self, ctx, exc):
        if isinstance(exc, AlreadyConnectedToChannel):
            emVar = discord.Embed(title="An Error Occurred",
                                  description="Already connected to a voice channel.",
                                  color=discord.Color.red())
            await ctx.send(embed=emVar)
        elif isinstance(exc, NoVoiceChannel):
            emVar2 = discord.Embed(title="An Error Occurred",
    ...
    async def play_command(self, ctx, *, query: t.Optional[str]):
        ...
        if query is None:
            ...
            await ctx.send("Resumed", delete_after=5.0)
    ...
    async def pause_command(self, ctx):
        ...
        await ctx.send("Paused", delete_after=5.0)
    ...
    async def stop_command(self, ctx):
        ...
        player.queue.empty()
        await player.stop()
        await ctx.send("Stopped", delete_after=5.0)
    ...
    async def next_command(self, ctx):
        ...
        await player.stop()
        emVar = discord.Embed(title="Skipped",
    ...
    async def previous_command(self, ctx):
        ...
        await player.stop()
        emVar = discord.Embed(title="Previous Track",
                              description="Playing the previous song in the queue",
                              color=discord.Color.dark_blue())
        await ctx.send(embed=emVar)
    ...
    async def shuffle_command(self, ctx):
        ...
        emVar = discord.Embed(title="Shuffled",
                              description="Started shuffle mode",
                              color=discord.Color.dark_blue())
        await ctx.send(embed=emVar)
    ...
    async def repeat_command(self, ctx, mode: str = "all"):
        if mode not in ("none", "1", "all"):
            raise InvalidRepeatMode

        player = self.get_player(ctx)
        player.queue.set_repeat_mode(mode)
        emVar = discord.Embed(title="Repeat Mode set",
    ...
    @commands.group(name="Volume", invoke_without_command=True, case_insensititve=True)
    ...
    async def playing_command(self, ctx):
        ...
        embed.set_footer(
            text=f"Requested by {ctx.author.display_name}", icon_url=ctx.author.avatar_url)
        embed.add_field(name="__Track title__",
                        value=player.queue.current_track.title, inline=False)
        embed.add_field(name="__Artist__",
                        value=player.queue.current_track.author, inline=False)
    ...
    @commands.command(name="Jump", aliases=["playindex"])
